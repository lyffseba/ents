...
1. **Dependency Pinning:** The curriculum env is isolated with `pixi`. Pins live in `max_env/pixi.toml` (`mojo==1.0.0`, `max==26.5.0`, `python 3.13`, plus JAX/MLX). The retired `modular` metapackage is not used.
2. **Model Safety:** Downloaded weights under `max_env/models/` are gitignored. Student MAX work compiles `max.graph` graphs, not pickled checkpoints.
