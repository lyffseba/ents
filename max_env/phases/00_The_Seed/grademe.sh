...
output00=$(cd ex00_jax_soil && python soil.py 2>/dev/null)
...
output01=$(cd ex01_max_roots && python roots.py 2>/dev/null)
if [[ "$output01" == *"[[ 0.5 -0.2  0.8  0.1]]"* ]] || [[ "$output01" == *"[[0.5 -0.2 0.8 0.1]]"* ]]; then
    echo "✅ PASS"
else
...
output02=$(cd ex02_mojo_sprout && mojo sprout.mojo 2>/dev/null)
